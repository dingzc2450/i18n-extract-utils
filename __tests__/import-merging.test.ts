...
import { NormalizedTransformOptions } from "../src/core/config-normalizer";
...
console.log('hello');`;
    ...
    const extractedStrings = [
      { key: "key1", value: "hello", filePath: "test.ts", line: 2, column: 12 },
    ];
    const importReq = { source: "lib", specifiers: [{ name: "b" }] };

    const modifiedCode = (processor as any).addOrMergeImports(
      code,
      [importReq],
      "test.ts"
    );
...
console.log('hello');`;
  ...
  it("should not merge imports if mergeImports is false", () => {
    const code = `import { a } from 'lib';
console.log('hello');`;
    const options: Pick<NormalizedTransformOptions, "normalizedI18nConfig"> = {
    ...
    const modifiedCode = (processor as any).addImportsAndHooks(
      code,
      [importReq],
      [],
      "test.ts",
