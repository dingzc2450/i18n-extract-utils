...
describe("错误处理单元测试", () => {
  ...
  describe("enhanceError错误增强测试", () => {
    test("应正确识别和增强解析错误", () => {
      const parseError = new Error(
        "Unexpected token (10:15) BABEL_PARSER_SYNTAX_ERROR"
      );
    ...
    test("应处理非Error类型对象", () => {
      ...
      // 使用字符串直接创建Error
      const directStringError = enhanceError(
        new Error("直接字符串"),
        "/test/path.js"
      );
  ...
  describe("中文错误信息测试", () => {
    ...
    test("应正确处理包含特殊字符的错误信息", () => {
      const specialError = createI18nError(
        "GENERAL001",
  ...
  describe("不同框架的错误处理", () => {
    ...
    test("Vue 模板错误处理", () => {
      const vueTemplateError = new Error(
        "Vue template compilation error: <template> tag has no matching end tag."
      );
      const enhanced = enhanceError(
        vueTemplateError,
        "/test/vue-component.vue"
      );
  ...
  describe("错误处理工具函数测试", () => {
    test("不同格式的行列信息提取", () => {
      ...
      const babelEnhanced = createI18nError("PARSING001", ["语法错误"], {
      ...
      const lineColumnEnhanced = createI18nError("PARSING001", ["语法错误"], {
        originalError: lineColumnError,
      });
      ...
      const lineOnlyEnhanced = createI18nError("PARSING001", ["语法错误"], {
        originalError: lineOnlyError,
      });
    ...
    test("不同参数数量的消息模板替换", () => {
      ...
      // 测试多参数情况
      const multiParamError = createI18nError("CONFIG001", [
        "param1",
        "param2",
        "param3",
      ]);
