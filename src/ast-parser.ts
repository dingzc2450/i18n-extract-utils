...
function createTranslationCall(methodName: string, translationKey: string): t.CallExpression {
...
export function transformCode(
  ...
  try {
    ...
    traverse(ast, {
      JSXAttribute(path) {
        if (path.node.value && t.isStringLiteral(path.node.value)) {
          ...
          if (match && match[1] !== undefined) { // Ensure capture group exists
            ...
            if (translationKey !== undefined) {
              ...
              // console.log(`[${filePath}] JSXAttribute Replaced.`); // Debugging
               ...
               console.warn(`[${filePath}] Warning: Found match "${textToTranslate}" in JSX attribute but no key in valueToKeyMap.`);
            }
          }
        }
      },
        ...
        if (match && match[1] !== undefined) {
          ...
          if (translationKey !== undefined) {
            ...
            // console.log(`[${filePath}] StringLiteral Replaced.`); // Debugging
             ...
             console.warn(`[${filePath}] Warning: Found match "${textToTranslate}" in StringLiteral but no key in valueToKeyMap.`);
          }
        }
      },
      JSXText(path) {
        ...
        while ((match = globalPattern.exec(textValue)) !== null) {
          ...
          if (translationKey !== undefined) {
            ...
            textModified = true; // Mark this node as modified
             ...
             console.warn(`[${filePath}] Warning: Found match "${textToTranslate}" in JSXText but no key in valueToKeyMap.`);
        ...
        if (lastIndex < textValue.length) {
           ...
           if (/\S/.test(remainingText)) { // Only add if it contains non-whitespace
        ...
        if (textModified && newNodes.length > 0) {
          ...
          if (newNodes.length === 1 && t.isJSXText(newNodes[0]) && newNodes[0].value === textValue) {
           ...
           if (match && match[1] !== undefined) {
             ...
             if (translationKey !== undefined) {
               ...
               // console.log(`[${filePath}] TemplateLiteral Replaced.`); // Debugging
                ...
                console.warn(`[${filePath}] Warning: Found match "${textToTranslate}" in TemplateLiteral but no key in valueToKeyMap.`);
             }
           }
         }
       },
          ...
          enter(path) {
            ...
            if (!importExists) {
              ...
              for (let i = 0; i < path.node.body.length; i++) {
                ...
                if (t.isExpressionStatement(node) && t.isStringLiteral(node.expression) && path.node.directives?.some(dir => dir.value.value === node.expression.value)) {
        ...
        "FunctionDeclaration|FunctionExpression|ArrowFunctionExpression": (path) => {
          ...
          path.traverse({
              ReturnStatement(returnPath) {
                  if (returnPath.node.argument && (t.isJSXElement(returnPath.node.argument) || t.isJSXFragment(returnPath.node.argument))) {
                      returnsJSX = true;
          ...
          if (returnsJSX && t.isFunction(path.node) && path.node.body && t.isBlockStatement(path.node.body)) {
            // Check if hook call already exists in this function
            let callExists = false;
            path.node.body.body.forEach((stmt) => {
              if (t.isVariableDeclaration(stmt)) {
                stmt.declarations.forEach((decl) => {
                  if (t.isVariableDeclarator(decl) && t.isObjectPattern(decl.id) && t.isCallExpression(decl.init) && t.isIdentifier(decl.init.callee) && decl.init.callee.name === hookName) {
                    callExists = true;
                  }
                });
              }
            ...
            if (!callExists) {
              ...
              const objectPattern = t.objectPattern([t.objectProperty(hookIdentifier, hookIdentifier, false, true)]);
    ...
    const transformedCode = formatGeneratedCode(generatedCode, importAdded, hookCallAdded);
    ...
    console.error(`[${filePath}] Falling back to simple regex replacement (key generation not supported in fallback)`);
    ...
    if (!hasHookAlready && extractedStrings.length > 0) {
        if (!transformedCode.includes(`import { ${hookName} } from '${hookImport}'`)) {
             ...
             for (let i = 0; i < lines.length; i++) {
                 ...
                 if (line.startsWith("'use ") || line.startsWith('"use ')) { directiveEndIndex = i; }
                 else if (line.startsWith('import ')) { lastImportIndex = i; }
                 else if (line && !line.startsWith('//') && !line.startsWith('/*') && !line.startsWith('*') && !line.startsWith('*/')) { break; }
             }
             let insertPosition = 0;
             if (lastImportIndex >= 0) { insertPosition = lastImportIndex + 1; }
             else if (directiveEndIndex >= 0) { insertPosition = directiveEndIndex + 1; }
             lines.splice(insertPosition, 0, `import { ${hookName} } from '${hookImport}';`);
             // Add blank lines logic if needed here too
             transformedCode = lines.join('\n');
        }
        const functionComponentRegex = /(function\s+\w+\s*\(.*?\)\s*\{|const\s+\w+\s*=\s*\(.*?\)\s*=>\s*\{)/g;
