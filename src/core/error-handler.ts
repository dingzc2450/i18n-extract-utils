...
const errorDefinitions: Record<string, ErrorDefinition> = {
  ...
  CONFIG003: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请检查 i18nConfig 配置，确保 framework 和 i18nImport 设置正确'
  },
  ...
  PARSING001: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请检查文件语法是否正确，特别是第 {1} 行附近。可能是语法错误或不支持的语法特性导致'
  },
  PARSING002: {
    code: 'PARSING002',
    category: ErrorCategory.PARSING,
    ...
    suggestionTemplate: '目前支持的文件类型有: .js, .jsx, .ts, .tsx, .vue。如需支持其他类型，请在配置中明确指定框架类型'
  },
  PARSING003: {
    code: 'PARSING003',
    category: ErrorCategory.PARSING,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '文件可能包含复杂或实验性语法，尝试使用 preserveFormatting: true 选项或简化代码结构'
  },
  ...
  TRANSFORM001: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '转换过程中出现错误，可能是由于复杂的代码结构导致，请简化相关代码或检查语法'
  },
  TRANSFORM002: {
    code: 'TRANSFORM002',
    category: ErrorCategory.TRANSFORMATION,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请确认要替换的字符串存在且格式正确，如果是动态生成的字符串可能需要手动处理'
  },
  TRANSFORM003: {
    code: 'TRANSFORM003',
    category: ErrorCategory.TRANSFORMATION,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '字符串替换位置计算错误，请尝试使用 contextAware: false 选项或使用 AST 转换模式'
  },
  TRANSFORM004: {
    code: 'TRANSFORM004',
    category: ErrorCategory.TRANSFORMATION,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请检查提取模式(pattern)配置是否正确，或者代码中的标记格式是否符合期望'
  },
  ...
  FILE003: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请提供有效的文件路径，确保路径格式正确且不包含非法字符'
  },
  ...
  PLUGIN001: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '插件 {1} 在处理过程中出错，请检查插件配置或相关代码结构是否符合插件要求'
  },
  PLUGIN002: {
    code: 'PLUGIN002',
    category: ErrorCategory.PLUGIN,
    ...
    severity: ErrorSeverity.WARNING,
    suggestionTemplate: '请在配置中明确指定框架类型(framework)，或确保文件内容符合自动检测标准'
  },
  PLUGIN003: {
    code: 'PLUGIN003',
    category: ErrorCategory.PLUGIN,
    ...
    severity: ErrorSeverity.WARNING,
    suggestionTemplate: '当前插件可能不完全支持所使用的框架版本，请尝试更新插件或调整配置'
  },
  ...
  VUE001: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请确认 Vue 组件格式正确，包含有效的 template/script/style 块，检查是否有语法错误'
  },
  VUE002: {
    code: 'VUE002',
    category: ErrorCategory.PLUGIN,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请检查 Vue 组件中的 script 部分语法是否正确，特别是 setup 语法的使用'
  },
  VUE003: {
    code: 'VUE003',
    category: ErrorCategory.PLUGIN,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请检查模板语法是否正确，确保模板表达式格式有效且闭合正确'
  },
  VUE004: {
    code: 'VUE004',
    category: ErrorCategory.PLUGIN,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: 'script setup 语法需要正确配置，请确认Vue版本兼容性并检查语法正确性'
  },
  ...
  REACT001: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请检查 React 组件语法是否正确，特别是 JSX 部分和组件结构'
  },
  REACT002: {
    code: 'REACT002',
    category: ErrorCategory.PLUGIN,
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '检查钩子使用是否符合规则，例如是否在组件顶层调用，是否有条件执行钩子等'
  },
  REACT003: {
    code: 'REACT003',
    category: ErrorCategory.PLUGIN,
  ...
  GENERAL001: {
    ...
    severity: ErrorSeverity.ERROR,
    suggestionTemplate: '请尝试简化代码或检查语法错误，如果问题持续存在，请提交问题报告'
  },
  GENERAL002: {
    code: 'GENERAL002',
    category: ErrorCategory.UNKNOWN,
    ...
    severity: ErrorSeverity.WARNING,
    suggestionTemplate: '处理过程耗时过长，可能是由于文件过大或代码过于复杂，尝试分解文件或优化代码结构'
  },
  GENERAL003: {
    code: 'GENERAL003',
    category: ErrorCategory.UNKNOWN,
...
export function createI18nError(
  ...
  // 从原始错误中提取更详细的信息
  let details = options.originalError ? options.originalError.message : undefined;
