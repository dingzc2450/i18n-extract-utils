...
export function collectContextAwareReplacementInfo(
  ...
  const extractOriginalText = (
    ...
    let result = lines[startLine - 1].slice(startColumn) + "\n";
    for (let i = startLine; i < endLine - 1; i++) {
      result += lines[i] + "\n";
    }
    result += lines[endLine - 1].slice(0, endColumn);
    return result;
  };
  ...
  traverse(ast, {
    StringLiteral(path) {
      // 如果这个字符串是 JSX 属性的值，跳过处理（会在 JSXAttribute 中处理）
      if (path.parentPath && isJSXAttribute(path.parentPath.node) && 
      ...
      if (matches.length === 1) {
        ...
        const callExpression = callFactory(importInfo.callName, key, standardizedValue);
